...
namespace ResortProjectAPI.Controllers
{
    ...
    public class AuthController : ControllerBase
    {
        ...
        public async Task<IActionResult> UserLogin([FromForm] LoginModel model)
        {
            ...
            return user.Password != model.Password ? BadRequest("Password was wrong") : Ok(new { token = GenerateJSONWebToken(model, "CLIENT") });
        }

        [HttpPost, Route("staff")]
        public async Task<IActionResult> StaffLogin([FromForm] LoginModel model)
        {
            if (!ModelState.IsValid) return BadRequest("Model is invalid: " + ModelState.Values);
            var staff = await _staff.GetById(model.Username);
            if (staff == null) return NotFound("Staff not exist");
            return staff.Password != model.Password ? BadRequest("Password was wrong") : Ok(new { token = GenerateJSONWebToken(model, staff.PermissionID) });
        }
